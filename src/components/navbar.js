...
class NavBar extends Component {
  ...
  renderNavItem() {
    return listItems.map((item, index) => {
      return (
        <div
          key={index}
          className={`nav_group ${this.state.activeIndex === "" + index ? "active" : ""}`}
        >
          <NavLink
            to={item.path}
            className="nav_item"
  ...
  handleParentClick(event) {
    if (this.state.isMobile) {
      if (
        event.target.dataset.id === this.state.activeIndex ||
  ...
  render() {
    return (
      <div
        className={`navbar ${this.state.tabActive && this.state.isMobile ? "active" : ""}`}
      >
        <div
          className={`hambuger ${this.state.tabActive ? "active" : ""}`}
          onClick={this.handleHambugerClick.bind(this)}
        >
          <img src={this.state.tabActive ? clear : hambuger} alt="-" />
        </div>
        <nav className="nav">{this.renderNavItem()}</nav>
      </div>
    );
  }
}
